...
function auth_googleoauth2_display_buttons() {
	...
	$html = html_writer::tag(
			'div',
			auth_googleoauth2_render_buttons(),
			array('class'=>'socialloginauth')
		);
	$showSteps = true;
	if($CFG->auth_instructions){
		...
		$html .= html_writer::tag(
			...
			array('class'=>'auth_instructions')
		);
	}
    $auth_instructions = $html;
	if ($CFG->registerauth == 'email' && $showSteps) {
		$auth_instructions .= get_string("loginsteps", "", "signup.php");
	}
	$CFG->auth_instructions = $auth_instructions;
}
...
function auth_googleoauth2_render_buttons() {
	global $CFG,$PAGE;
	$html ='';
	$filename = '/auth/googleoauth2/csssocialbuttons/css/zocial.css';
	$PAGE->requires->css($filename);
...
//	}
	...
	if($displayprovider){
		$html .= '
				<a title="'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'" class="zocial googleplus" href="https://accounts.google.com/o/oauth2/auth?client_id='.
	...
	if($displayprovider){
		...
		$html .= '
				<a title="'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'" class="zocial facebook" href="https://www.facebook.com/dialog/oauth?client_id='. get_config('auth/googleoauth2', 'facebookclientid') .'&redirect_uri='. $CFG->wwwroot .'/auth/googleoauth2/facebook_redirect.php&state='.auth_googleoauth2_get_state_token().'&scope=email&response_type=code">
					'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'
				</a>
			';
	}
	...
	if($displayprovider){
		...
		$html .= '
				<a title="'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'" class="zocial github" href="https://github.com/login/oauth/authorize?client_id='. get_config('auth/googleoauth2', 'githubclientid') .'&redirect_uri='. $CFG->wwwroot .'/auth/googleoauth2/github_redirect.php&state='.auth_googleoauth2_get_state_token().'&scope=user:email&response_type=code">
					'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'
				</a>
			';
	}
	...
	if($displayprovider){
		...
		$html .= '
				<a title="'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'" class="zocial linkedin" href="https://www.linkedin.com/uas/oauth2/authorization?client_id='. get_config('auth/googleoauth2', 'linkedinclientid') .'&redirect_uri='. $CFG->wwwroot .'/auth/googleoauth2/linkedin_redirect.php&state='.auth_googleoauth2_get_state_token().'&scope=r_basicprofile%20r_emailaddress&response_type=code">
					'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'
				</a>
			';
	}
	...
	if($displayprovider){
		...
		$html .= '
				<a title="'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'" class="zocial windows" href="https://oauth.live.com/authorize?client_id='. get_config('auth/googleoauth2', 'messengerclientid') .'&redirect_uri='. $CFG->wwwroot .'/auth/googleoauth2/messenger_redirect.php&state='.auth_googleoauth2_get_state_token().'&scope=wl.basic wl.emails wl.signin&response_type=code">
					'.get_string('auth_sign-in_with','auth_googleoauth2', $a).'
				</a>
		';
	}
