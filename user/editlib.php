...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    global $CFG, $USER, $DB;
    $mform->addElement('static', 'myusername', get_string('username'), $user->username);
    if ($user->id > 0) {
    ...
    /* BEGIN - make country required https://support.informea.org/issues/670 */
    $mform->addRule('country', get_string('required'), 'required', null, 'client');
    /* END - make country required */
