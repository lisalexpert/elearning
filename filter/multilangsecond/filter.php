...
class filter_multilangsecond extends moodle_text_filter {
    ...
    public function filter($text, array $options = array()) {
        ++self::$counter;
        global $CFG;
        // echo 'filtering ' . $counter .  '<pre>'.$text.'</pre>';
        ...
        if ($CFG->filter_multilangsecond_mode){
			// $search = '/.*(?:\{mlang\s+[a-z0-9_]+\}[^{]*\{mlang\}).*+/is';
            $search = '/(?:\{mlang\s+.+?\}[^{]*\{mlang\}){1,}/is';
	...
	if (!$skip_2 && $CFG->filter_multilangsecond_mode > 1){
        $res = $result;
...
function filter_multilangsecond_impl($langblock) {
	...
	// $searchtosplit = '/\{mlang\s+([a-z0-9_]+)\}(.*?)\{mlang\}/is';
    ...
    if(isset($langlist['cn'])){
        $langlist['zh_cn'] = $langlist['cn'];
    }
    if (array_key_exists($mylang, $langlist)) {
		...
		if($CFG->lang){
			if (array_key_exists($CFG->lang, $langlist)) {
				return $langlist[$CFG->lang];
			}
			return "";
		}
